...
Note entonces que la frontera de clasificación esta en una linea que toma solo dos puntos de la clase 1 y el resto de puntos los toma como clase 0. Claramente, dentro del conjunto de entrenamiento, esto no es una buena clasificación para este conjunto de datos.
...
glm_test_syntetic_quadratic_class <- tibble(estimate = as.factor(response_predict_test_syntetic_quadratic),
                        truth = as.factor(syntetic_quadratic_test$y))
...
En este punto, la matriz de confusión revela que el modelo tiene especial dificultad en el reconocimiento correcto de datos cuya etiqueta dentro del conjunto de datos es 0, es decir, hay abundancia de falsos negativos.
...
```{r}
svm_linear_syntetic_quadratic <- svm(y ~ ., data = syntetic_quadratic_train, kernel = "linear", cost = 10, scale = FALSE)
...
```{r}
predict_test_svm_syntetic_quadratic <- predict(svm_linear_syntetic_quadratic, syntetic_quadratic_test)
...
```{r}
svm_nonlinear_syntetic_quadratic <- svm(y ~ ., data = syntetic_quadratic_train, kernel = "polynomial", degree = 2,  cost = 10)
...
```{r}
predict_svm_nonlinear_syntetic_quadratic <- predict(svm_nonlinear_syntetic_quadratic, syntetic_quadratic_test)
...
```{r}
set.seed(20)
tune_svc <- tune(svm, mpg01 ~ . - name, data = auto, kernel = "linear", 
...
```{r}
set.seed(20)
tune_svm_polynomial <- tune(svm, mpg01 ~ . - name, data = auto, kernel = "polynomial", probability = TRUE,
...
```{r}
predictions <- predict(tune_svm_polynomial$best.model, auto_test, probability = TRUE)
roc <- tibble(estimate = predictions, 
              truth = auto_test$mpg01,
              prob = attr(predictions,"probabilities")[,2])
roc_curve(roc, truth, prob) %>% autoplot()
```
...
```{r}
set.seed(20)
tune_svm_radial <- tune(svm, mpg01 ~ . - name, data = auto, kernel = "radial", 
