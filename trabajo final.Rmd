...
partition <-  sample.split(auto, SplitRatio = split_ratio)
auto_train <- subset(auto, partition == TRUE)
auto_test <- subset(auto, partition == FALSE)
```
...
get_accuracy_knn <- function(k) {
  ...
  mpg_knn_class <- tibble(pred = as.factor(knn_model), real = auto_test$mpg01)
  test_results_knn <- mpg_knn_class %>% 
    metrics(truth = real, estimate = pred)
  
  return(test_results_knn$.estimate[1])
}
```
...
best_accuracy_knn = round(get_accuracy_knn(best_k) * 100, 2)
```
